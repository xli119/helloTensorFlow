...
# Using Interactive session makes is the default sessions so we do not need to pass sess
sess = tf.InteractiveSession()

# Define placeholder for MNIST input data
x = tf.placeholder(tf.float32, shape=[None, 784])
y_ = tf.placeholder(tf.float32, [None, 10])

# change the MNIST input data from a list of values to a 28*28 pixel grayscale value cube
...
# 32 features for each 5*5 patch of image
...
for i in range(num_steps):
    ...
    if i % display_every == 0:
        train_accuracy = accuracy.eval(feed_dict={
            x:batch[0], y_: batch[1], keep_prob: 0.1
        })
        end_time = time.time()
        print("step{0}, elapsed time {1: .2f} second, training accuracy {2:.3f}%".format(i,
